...
def create_aligned_counts_by_image(alignment_register):
    ...
    aligned_counts_by_image = {
        entry["filename"]: {gt_id: {"total_aligned_lines_count": 0,
                                    "aligned_clusters_size": []} for gt_id in gt_ids}
    ...
    for entry in alignment_register:
        ...
        aligned_counts_by_image[filename][gt_id]["total_aligned_lines_count"] = total_aligned_lines_count
        aligned_counts_by_image[filename][gt_id]["aligned_clusters_size"] = aligned_clusters_size
...
def identify_top_n_best_gt_based_on_total_alg_lines(aligned_counts_by_image, n_best_gt):
    ...
    for filename, gt_counts in aligned_counts_by_image.items():
        ...
        sorted_gt_counts = sorted(gt_counts.items(
        ), key=lambda x: x[1]["total_aligned_lines_count"], reverse=True)
        # Take top n GTs
        top_n_gt_counts = sorted_gt_counts[:n_best_gt]
        top_n_best_gt[filename] = [
            (gt_id, counts["total_aligned_lines_count"]) for gt_id, counts in top_n_gt_counts]
    return top_n_best_gt


def identify_top_n_best_gt_based_on_biggest_cluster_size(aligned_counts_by_image, n_best_gt):
    """ 
    ...
    for filename, gt_counts in aligned_counts_by_image.items():
        ...
        sorted_gt_counts = sorted(gt_counts.items(), key=lambda x: max(
            x[1]["aligned_clusters_size"], default=0), reverse=True)
        # Take top n GTs
        top_n_gt_counts = sorted_gt_counts[:n_best_gt]
        top_n_best_gt[filename] = [(gt_id, max(
            counts["aligned_clusters_size"], default=0)) for gt_id, counts in top_n_gt_counts]
    return top_n_best_gt
...
def create_tsv_total_alg_lines(aligned_counts_by_image, doc_pk, top_n_best_gt, display_n_best_gt, n_best_gt):
    """
    ...
    """
    # Collect all unique GT_ids
    gt_ids = sorted(set(gt_id for gt_counts in aligned_counts_by_image.values()
    ...
    if display_n_best_gt:
        tsv_header += "\t".join(
            f"best_GT_{i}_id\tbest_GT_{i}_aligned_lines_count" for i in range(1, n_best_gt + 1)) + "\t"
    ...
    for filename, gt_counts in aligned_counts_by_image.items():
        ...
        for gt_id in gt_ids:
            total_aligned_lines_count = gt_counts.get(
                gt_id, {}).get("total_aligned_lines_count", '')
...
def create_tsv_biggest_cluster_size(aligned_counts_by_image, doc_pk, top_n_best_gt, display_n_best_gt, n_best_gt):
    """
    ...
    if display_n_best_gt:
        tsv_header += "\t".join(
            f"best_GT_{i}_id\tbest_GT_{i}_biggest_cluster_size" for i in range(1, n_best_gt + 1)) + "\t"
    tsv_header += "\t".join(gt_ids) + "\n"
    ...
    for filename, gt_counts in aligned_counts_by_image.items():
        ...
        for gt_id in gt_ids:
            biggest_cluster_size = max(gt_counts.get(gt_id, {}).get(
                "aligned_clusters_size", [0]), default=0)
...
def create_tsv_total_alg_lines_from_alignment_register(alignment_register, doc_pk, display_n_best_gt=True, n_best_gt=1):
    """
    ...
    """
    aligned_counts_by_image = create_aligned_counts_by_image(
        alignment_register)
    ...
    tsv_file_path = os.path.join(
        results_summary_tsv_path, f"results_based_on_total_alg_lines_{doc_pk}_n{n}_lev_{levenshtein_threshold}.tsv")
    with open(tsv_file_path, "w") as tsv_file:
        tsv_file.write(tsv_content)


def create_tsv_biggest_cluster_size_from_alignment_register(alignment_register, doc_pk, display_n_best_gt=True, n_best_gt=1):
    """
    Global function that creates a TSV file with the number of aligned lines for each GT and image.
    This function can give the n best GT for each image.
    Rows: one row per image (filename from the XML file)
    ...
    # Write to file in the results directory
    tsv_file_path = os.path.join(
        results_summary_tsv_path, f"results_based_on_biggest_cluster_size_{doc_pk}_n{n}_lev_{levenshtein_threshold}.tsv")
    with open(tsv_file_path, "w") as tsv_file:
        tsv_file.write(tsv_content)
