...
def clean_alg_text(alg_text):
    ...
    alg_text = alg_text.replace(
        '-', '') if alg_text.replace('-', '') else alg_text
...
def extract_passim_results(passim_out_json_path):
    ...
    for GT_id in GT_ids:
        ...
        for textblock in out_passim_list:
            ...
            filename = re.sub(r'.*' + textblock_id + '_(.*)',
                              r'\1', textblock['id'])
...
def count_aligned_line_clusters(lines_dict, filename, levenshtein_threshold):
    ...
    # Parcours chaque partie du dictionnaire de lignes
    for part in lines_dict:
        if part['filename'] != filename:
            continue  # Passe à la partie suivante si le nom de fichier ne correspond pas

        # Parcours chaque bloc OCR dans la partie actuelle
        for block in part['ocr_blocks']:

            # Parcours chaque ligne OCR dans le bloc actuel
            for line in block['ocr_lines']:
                if line.get('alg_GT') and line['levenshtein_ratio'] >= levenshtein_threshold:
                    # Incrémente le compteur de lignes alignées successives
                    successive_aligned_lines += 1
                else:
                    # Si le compteur de lignes successives est supérieur à 0, ajoute le nombre de lignes au cluster
                    if successive_aligned_lines > 0:
                        clusters.append(successive_aligned_lines)
                    # Réinitialise le compteur
                    successive_aligned_lines = 0

            # Si on termine un bloc avec des lignes successives alignées, les ajouter aussi
            if successive_aligned_lines > 0:
                clusters.append(successive_aligned_lines)
                successive_aligned_lines = 0
...
def process_alignment_xml_as_txt(lines_dict_with_alg_GT_path, xmls_for_eSc_path, xmls_from_eSc_path, levenshtein_threshold):
    ...
    for json_file in os.listdir(lines_dict_with_alg_GT_path):
        if json_file.endswith('.json'):
            with open(os.path.join(lines_dict_with_alg_GT_path, json_file), 'r', encoding="utf-8") as json_file_handler:
            ...
            for xml_file in os.listdir(xmls_from_eSc_path):
                if xml_file.endswith('.xml'):
                    ...
                    with open(os.path.join(xmls_from_eSc_path, xml_file), encoding="utf-8") as xml_file_handler:
                    ...
                    for xml_text_line in xml_text_lines:
                        xml_text_line_id = re.search(
                            r'<TextLine ID="(.*?)"', xml_text_line).group(1)
                        if xml_text_line_id not in [line_id for line_id, _, _ in line_ids_with_GT]:
                            updated_content = re.sub(
                                r'<String CONTENT=".*?"', '<String CONTENT=""', xml_text_line)
                                ...
                                if text_line_id == xml_text_line_id:
                                    ...
                                    if string_content_match:
                                        if levenshtein_ratio >= levenshtein_threshold:
                                            ...
                                            new_content = 'CONTENT=""'
                                        xml_as_txt = xml_as_txt.replace(
                                            string_content_match.group(0), f'<String {new_content}')

                    if line_count > 0:
                        filename = os.path.splitext(xml_file)[0]
                        part_pk = get_pk_from_filename(
...
def process_passim_results(passim_out_json_path, lines_dict_with_alg_GT_path, xmls_from_eSc_path, xmls_for_eSc_path, levenshtein_threshold):
    '''
    ...
    process_alignment_xml_as_txt(
        lines_dict_with_alg_GT_path, xmls_for_eSc_path, xmls_from_eSc_path, levenshtein_threshold)
