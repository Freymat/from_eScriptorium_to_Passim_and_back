...
def extract_ocr_textblocks(xmls_from_eSc_path, ocr_lines_dict_path):
    ...
    for filename in glob.glob(os.path.join(xmls_from_eSc_path, "*.xml")):
        ...
        for text_block in root.iter(
            ...
            for text_line in text_block.iter(
                "{http://www.loc.gov/standards/alto/ns-v4#}TextLine"
            ):
                text = (
                    text_line.find("{http://www.loc.gov/standards/alto/ns-v4#}String")
                    .get("CONTENT")
                    .strip()
                )
                ...
                char_count += len(
                    text + separator
                )  # Updates the starting position for the next line.
        ...
        parts.append(
            {
                "filename": basename,
                "ocr_lines_in_part": sum(
                    [block["ocr_lines_in_block"] for block in blocks]
                ),
                "ocr_blocks": blocks,
        ...
        for file in files:
            if file.endswith(".txt"):
                ...
                with open(text_file, "r", encoding="utf-8") as file_handler:
                    ...
                    passim_input.append(
                        {"id": filename, "series": "GT", "ref": "1", "text": text}
                    )
...
def build_passim_input(
    xmls_from_eSc_path, ocr_lines_dict_path, GT_texts_directory_path, input_passim_path
):
    """
